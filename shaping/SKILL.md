---
name: shaping
description: Use this methodology when collaboratively shaping a solution with the user - iterating on problem definition (requirements) and solution options.
---
...
**Shaping doc** is where active work happens. All exploration, requirements gathering, option comparison, breadboarding, and coverage checking happens here. This is the working document and ground truth for R, options, parts, and coverage checks.
...
After an option is breadboarded, slice it into implementation steps. Use the `/breadboarding` skill for the slicing process — it defines what implementation steps are, the procedure for creating them, and visualization formats.
