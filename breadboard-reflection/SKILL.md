...
**6. State that co-accesses suggests Places.** Which functions and state are used together but don't touch other parts of the system? If `loading`, `TOOLS`, and `SYSTEM_PROMPT` are all accessed by the command input flow and never by the table display, they cluster into a candidate Place. Place boundaries emerge from co-access patterns, not just from UI layout.
